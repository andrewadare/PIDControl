...
void printStatus() {
  String s = String("kp,ki,kd: ") + kp + ", " + ki + ", " + kd +
             ...
             String(" setpoint: ") + pid.setpoint + String(" output: ") +
             pid.output + String(" pwm: ") + dutyCycle;
  Serial.println(s);
}
...
void handleByte(byte b) {
  ...
  if (b == '\r' || b == '\n') {
    pid.setpoint =
        constrain((float)cmd.toInt() / 1023, pid.minOutput, pid.maxOutput);
...
void loop() {
  ...
  // From the PID output value (which is in the -1 to +1 range), compute an
  // 8-bit PWM duty cycle value. Scale up by 1e4 to improve accuracy of integer
  // math.
